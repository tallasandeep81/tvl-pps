...
/* Several people can share one slot in forging and casting. They are stored in
   the one OPERATOR cell, joined by " + ". */
const OP_SEP = ' + ';
function opList(v) {
...
function renderCell(dept, res, day, shift) {
  ...
  /* operator — one person, or a team where the department allows it */
  if (dept.hasOperator) {
    const chosen = opList(c.operator);

    const commit = list => {
      c.operator = opJoin(list);
      touch(td, day, shift, res.id);
      render();
      if (cellReady(c)) askRepeat(day, shift, res.id);
    };

    const picker = el('select', { class: 'op' });
    picker.appendChild(el('option', {
      value: '',
      text: dept.multiOperator
        ? (chosen.length ? '+ add ' + dept.operatorLabel.toLowerCase() : '— nobody assigned —')
        : '— not assigned —'
    }));

    const groups = new Map();
    Store.operators(S.dept).forEach(o => {
      if (dept.multiOperator && chosen.indexOf(o.name) >= 0) return;   // already on this slot
    ...
    ordered.forEach(g => {
      const parent = single ? picker : el('optgroup', { label: g });
      groups.get(g).forEach(o => {
        const opt = el('option', {
          value: o.name, text: opLabel(o.name),
          title: o.name + (o.section ? '  —  ' + o.section : '')
        });
        if (!dept.multiOperator && o.name === c.operator) opt.selected = true;
        parent.appendChild(opt);
      });
      if (!single) picker.appendChild(parent);
    });

    picker.addEventListener('change', async () => {
      if (picker.value === ADD_NEW) {
        picker.value = dept.multiOperator ? '' : c.operator;
        try {
          const name = await Store.addOperator(S.dept, shift);
          if (name) commit(dept.multiOperator ? chosen.concat(name) : [name]);
        } catch (e) { toast(e.message, 'err'); }
        return;
      }
      if (dept.multiOperator) {
        if (!picker.value) return;
        commit(chosen.concat(picker.value));
      } else {
        commit(picker.value ? [picker.value] : []);
      }
    });

    picker.appendChild(el('option', { value: ADD_NEW, text: '+ Add new ' + dept.operatorLabel.toLowerCase() + '…' }));

    if (dept.multiOperator && chosen.length) {
      const crew = el('div', { class: 'crew' });
      chosen.forEach(nm => {
        const chip = el('span', { class: 'chip' });
        chip.appendChild(el('b', { text: opLabel(nm), title: nm }));
        chip.appendChild(el('button', {
          class: 'x', text: '\u00d7', title: 'Remove ' + nm,
          onclick: () => commit(chosen.filter(x => x !== nm))
        }));
        crew.appendChild(chip);
      });
      stack.appendChild(crew);
    }
